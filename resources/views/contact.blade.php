...
@php
    $whatsappNum = !empty($cards['whatsapp_num']) ? $cards['whatsapp_num'] : \App\Models\SiteSetting::get('whatsapp_number', '917016266727');
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12 sm:mb-16 space-y-3.5">
            <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-none bg-white border border-[#E5DFD7] shadow-2xs text-[10.5px] font-semibold uppercase tracking-[0.22em] text-[#8C7A6B]">
                ...
                <span>{{ $header['badge'] ?? 'Get in Touch' }}</span>
            </div>
            
            <h1 class="font-serif text-3xl sm:text-4xl lg:text-5xl font-medium text-[#171412] tracking-tight leading-[1.15]">
                {{ $header['title_prefix'] ?? 'The' }} <span class="italic font-normal text-[#A33B47]">{{ $header['title_highlight'] ?? 'Concierge' }}</span> {{ $header['title_suffix'] ?? 'Lounge' }}
            </h1>
            
            <p class="text-xs sm:text-sm text-[#6A625A] font-light max-w-md mx-auto leading-relaxed">
                {{ $header['subtitle'] ?? 'Our Parisian studio specialists are ready to assist with custom nail sizing, bespoke press-on designs, and express order dispatch.' }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
            ...
            <div class="lg:col-span-5 space-y-4 sm:space-y-5">
                ...
                <div class="bg-white rounded-none p-6 border border-[#ECE6DE] shadow-2xs hover:shadow-md hover:border-[#171412] transition-all duration-300 flex gap-4 sm:gap-5 items-start group">
                    ...
                    <div>
                        <h3 class="font-serif font-semibold text-[#171412] text-base mb-1">{{ $cards['whatsapp_title'] ?? 'WhatsApp Concierge' }}</h3>
                        <p class="text-xs text-[#736B63] font-light mb-3 leading-relaxed">{{ $cards['whatsapp_desc'] ?? 'Chat directly with our studio artists for instant sizing help and real-time guidance.' }}</p>
                        <a href="https://wa.me/{{ $whatsappNum }}" target="_blank" rel="noopener noreferrer"
                           class="inline-flex items-center gap-1.5 text-xs font-semibold text-[#171412] hover:text-[#A33B47] transition-colors group/link">
                            <span>{{ $cards['whatsapp_btn_text'] ?? 'Open Direct WhatsApp' }} (+{{ $whatsappNum }})</span>
                ...
                <div class="bg-white rounded-none p-6 border border-[#ECE6DE] shadow-2xs hover:shadow-md hover:border-[#171412] transition-all duration-300 flex gap-4 sm:gap-5 items-start group">
                    ...
                    <div>
                        <h3 class="font-serif font-semibold text-[#171412] text-base mb-1">{{ $cards['email_title'] ?? 'Email Atelier' }}</h3>
                        <p class="text-xs text-[#736B63] font-light mb-3 leading-relaxed">{{ $cards['email_desc'] ?? 'For wholesale inquiries, press collaborations, and custom bridal suites.' }}</p>
                ...
                <div class="bg-white rounded-none p-6 border border-[#ECE6DE] shadow-2xs hover:shadow-md hover:border-[#171412] transition-all duration-300 flex gap-4 sm:gap-5 items-start group">
                    ...
                    <div>
                        <h3 class="font-serif font-semibold text-[#171412] text-base mb-1">{{ $cards['address_title'] ?? 'Paris Atelier' }}</h3>
                ...
                <div class="bg-[#171412] rounded-none p-6 text-[#FAF8F5] border border-[#26221E] shadow-sm">
                    <h3 class="font-serif font-semibold text-base mb-4 text-[#FAF8F5] flex items-center justify-between">
                        <span>{{ $cards['hours_title'] ?? 'Concierge Hours' }}</span>
                        <span class="text-[10px] font-sans font-semibold tracking-wider text-[#C5A880] uppercase">{{ $cards['hours_tz'] ?? 'Paris Time' }}</span>
                    </h3>
                    <div class="space-y-2.5 text-xs">
                        <div class="flex justify-between border-b border-white/10 pb-2">
                            <span class="text-[#A89F97]">Monday – Friday</span>
                            <span class="font-medium text-[#FAF8F5]">{{ $cards['hours_mon_fri'] ?? '9:00 AM – 8:00 PM' }}</span>
                        </div>
                        <div class="flex justify-between border-b border-white/10 pb-2">
                            <span class="text-[#A89F97]">Saturday</span>
                            <span class="font-medium text-[#FAF8F5]">{{ $cards['hours_sat'] ?? '10:00 AM – 6:00 PM' }}</span>
                        </div>
                        <div class="flex justify-between pb-1">
                            <span class="text-[#A89F97]">Sunday</span>
                            <span class="font-medium text-[#FAF8F5]">{{ $cards['hours_sun'] ?? '12:00 PM – 5:00 PM' }}</span>
                        </div>
                    </div>
                    <div class="mt-4 pt-3 border-t border-white/10 flex items-center gap-2 text-xs text-[#25D366]">
                        ...
                        <span>{{ $cards['hours_response_badge'] ?? 'WhatsApp response typically under 2 minutes' }}</span>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white rounded-none border border-[#ECE6DE] shadow-xs p-7 sm:p-9 space-y-7">
                    ...
                    <div>
                        <div class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-none bg-[#FAF8F5] border border-[#E5DFD7] text-[10.5px] font-semibold uppercase tracking-[0.2em] text-[#8C7A6B] mb-3">
                            <span class="text-[#A33B47]">✦</span>
                            <span>{{ $topics_widget['badge'] ?? 'Direct Specialist Connection' }}</span>
                        </div>
                        <h2 class="font-serif text-2xl sm:text-3xl font-medium text-[#171412] tracking-tight">
                            {{ $topics_widget['title_prefix'] ?? 'Choose Your' }} <span class="italic font-normal text-[#A33B47]">{{ $topics_widget['title_highlight'] ?? 'Inquiry Topic' }}</span>
                        </h2>
                        <p class="text-xs text-[#736B63] mt-2 font-light leading-relaxed">
                            {{ $topics_widget['subtitle'] ?? 'Select the topic that best fits your needs, and our studio artists will prepare customized answers before your WhatsApp chat opens.' }}
                        </p>
                    </div>
                        ...
                        <div class="flex flex-wrap gap-2.5">
                            <template x-for="t in topics" :key="t.id">
                                <button type="button" @click="selectedTopic = t.label"
                                        ...
                                        class="px-4 py-2.5 rounded-none border text-xs tracking-wider transition-all duration-200 uppercase cursor-pointer"
                    ...
                    <a :href="whatsappUrl" target="_blank" rel="noopener noreferrer"
                       class="w-full py-4 bg-[#171412] hover:bg-black text-white rounded-none font-semibold text-xs tracking-[0.18em] uppercase shadow-xs hover:shadow-md flex items-center justify-center gap-2.5 transition-all duration-300 group">
                        ...
                        <span>{{ $topics_widget['btn_text'] ?? 'Start WhatsApp Conversation' }}</span>
                    ...
                    <div class="grid grid-cols-3 gap-2.5 sm:gap-3 text-center">
                        <div class="text-[10.5px] text-[#736B63] font-normal bg-[#FAF8F5] border border-[#ECE6DE] rounded-none py-2.5 px-1 flex items-center justify-center gap-1.5">
                            ...
                            <span>{{ $topics_widget['badge1_text'] ?? 'Direct Artists' }}</span>
                        </div>
                        <div class="text-[10.5px] text-[#736B63] font-normal bg-[#FAF8F5] border border-[#ECE6DE] rounded-none py-2.5 px-1 flex items-center justify-center gap-1.5">
                            ...
                            <span>{{ $topics_widget['badge2_text'] ?? '< 2-Min Reply' }}</span>
                        </div>
                        <div class="text-[10.5px] text-[#736B63] font-normal bg-[#FAF8F5] border border-[#ECE6DE] rounded-none py-2.5 px-1 flex items-center justify-center gap-1.5">
                            ...
                            <span>{{ $topics_widget['badge3_text'] ?? 'Zero Waiting' }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
function conciergeWidget() {
    ...
    const topicsList = @json($activeTopics);
    return {
        selectedTopic: topicsList.length > 0 ? topicsList[0].label : 'Sizing & Curve Fit',
        topics: topicsList,
        get currentMessage() {
        ...
        get whatsappUrl() {
            const t = this.topics.find(t => t.label === this.selectedTopic) || this.topics[0];
            const msg = t ? t.msg : '';
            return 'https://wa.me/' + defaultNumber + '?text=' + encodeURIComponent(msg);
        }
    };
}
</script>
